...
export type TransformChangeEvent = CustomEvent<{
	x: number;
	y: number;
	scale: number;
	matrix: DOMMatrix;
...
export class Panzoom extends TypedEventTarget<PanzoomEventMap> {
	...
	private onPointerMove(previousPointers: Pointer[], currentPointers: Pointer[]) {
	...
	private updateTransform(scale: number, x: number, y: number) {
